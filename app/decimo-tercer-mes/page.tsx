...
import { Check, Loader2, Save, Trash2, Calculator, DollarSign, Eye, Download, FileText, Calendar, AlertCircle } from "lucide-react"
...
export default function DecimoTercerMesPage() {
    ...
    // --- KPIs Calculations ---
    const totalDecimoNeto = useMemo(() => calculatedDecimos.reduce((sum, item) => sum + item.montoNeto, 0), [calculatedDecimos])
    const totalDecimoBruto = useMemo(() => calculatedDecimos.reduce((sum, item) => sum + item.montoTotal, 0), [calculatedDecimos])
        ...
        if (entriesForCurrentYear.length > 0) {
            const syncedData: CalculatedDecimo[] = entriesForCurrentYear.map(e => {
                ...
                return {
                    ...
                    employeeName: employee ? `${employee.nombre} ${employee.apellido}` : 'Empleado Desconocido'
                }
    ...
    const handleDownloadPDFs = (type: 'all' | 'voucher_abr' | 'voucher_ago' | 'voucher_dic' | 'full_pdf', decimo?: CalculatedDecimo) => {
        if (calculatedDecimos.length === 0) {
            toast({ title: "Error", description: "Primero debe calcular el décimo tercer mes", variant: "destructive", })
            return
        }
        ...
        if (type === 'all') {
            ...
            message = `Descargando ${type.replace('_', ' ')} para ${decimo.employeeName} (Función simulada).`;
        } else {
             message = 'Descarga de comprobantes (Función simulada).';
        }
        
        toast({ title: "Descarga Solicitada", description: message, variant: "default"})
    }
    ...
    return (
        <div className="p-8">
            ...
            <div className="grid gap-6 md:grid-cols-3 mb-6">
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium">Monto Total Bruto</CardTitle>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium">Monto Neto por Pago</CardTitle>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button onClick={handleCalculateDecimo} className="gap-2" disabled={isCalculating || !currentCompany || activeEmployees.length === 0 || isYearClosed}>
                        {isCalculating ? (
            ...
            <CardContent>
                ...
                <div className="text-sm text-muted-foreground pt-4">
                    {entriesForCurrentYear.length > 0 && (
                        <span>
                            Estado: <Badge variant={isYearClosed ? "default" : "secondary"}>{isYearClosed ? "Pagado Completo" : "Cálculo Existente"}</Badge>
                        </span>
                    )}
                </div>
            </CardContent>
                ...
                <Card className="mb-6">
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Décimo tercer mes calculado para el año {currentYear}</CardDescription>
                            </div>
                            <div className="flex gap-2">
                                <Button onClick={() => handleDownloadPDFs('all')} variant="outline" className="gap-2" disabled={isCalculating}>
                                    <Download className="h-4 w-4" />
                                    Descargar Todos
                                </Button>
                                <Button onClick={() => handleSaveDecimo('pagado_completo')} className="gap-2" disabled={isSaving || isYearClosed}>
                    ...
                    <CardContent>
                        ...
                        <div className="rounded-lg bg-primary/10 p-6 text-center mb-6">
                            <p className="text-sm text-muted-foreground mb-2">Monto Neto Total a Pagar</p>
                            <p className="text-4xl font-bold text-primary">
                                {formatCurrency(totalDecimoNeto)}
                            </p>
                            <p className="text-xs text-muted-foreground mt-2">Monto después de deducciones</p>
                        </div>
                            ...
                            <TableBody>
                                {activeEmployees.length === 0 ? (
                                    ...
                                    calculatedDecimos.map((calc) => {
                                        const employeeData = employees.find(e => e.id === calc.empleadoId);
                                        return (
                                            <TableRow key={calc.empleadoId}>
                                                <TableCell className="font-medium">{calc.employeeName}</TableCell>
                                                <TableCell className="font-mono">{employeeData?.cedula || 'N/A'}</TableCell>
                                                    ...
                                                    <div className="flex items-center justify-center gap-1">
                                                        ...
                                                        <DropdownMenu>
                                                            ...
                                                            <DropdownMenuContent align="end">
                                                                ...
                                                                <DropdownMenuItem onClick={() => handleDownloadPDFs('full_pdf', calc)}>
                                                                    <FileText className="h-4 w-4 mr-2" />
                                                                    Comprobante Completo
                                                                </DropdownMenuItem>
                                                                <DropdownMenuSeparator />
                                                                <DropdownMenuItem onClick={() => handleDownloadPDFs('voucher_abr', calc)}>Voucher Abril</DropdownMenuItem>
                                                                <DropdownMenuItem onClick={() => handleDownloadPDFs('voucher_ago', calc)}>Voucher Agosto</DropdownMenuItem>
                                                                <DropdownMenuItem onClick={() => handleDownloadPDFs('voucher_dic', calc)}>Voucher Diciembre</DropdownMenuItem>
                                                            </DropdownMenuContent>
                                                        </DropdownMenu>
                                                    </div>
                                                </TableCell>
            ...
            <Dialog open={!!selectedEmployeeDetail} onOpenChange={() => setSelectedEmployeeDetail(null)}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>
                            Detalle de Décimo Tercer Mes - {selectedEmployeeDetail?.employeeName}
                        </DialogTitle>
                        <DialogDescription>Desglose completo del cálculo para el año {currentYear}</DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-4 pt-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-lg font-semibold">{formatCurrency(selectedEmployeeDetail.salarioPromedio)}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-muted-foreground">Meses Trabajados</p>
                                    <p className="text-lg font-semibold">{selectedEmployeeDetail.mesesTrabajados}</p>
                                </div>
                            </div>
                            <Separator />
                            <div className="space-y-2">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-mono font-semibold">{formatCurrency(selectedEmployeeDetail.montoTotal)}</span>
                                </div>
                                <div className="flex justify-between items-center text-red-600 dark:text-red-400">
                                    <span className="text-sm">Total Deducciones (CSS + ISR)</span>
                                    <span className="font-mono">-{formatCurrency(selectedEmployeeDetail.totalDeducciones)}</span>
                                </div>
                                <div className="flex justify-between items-center font-semibold">
                                    <span>Monto Neto Final</span>
                                    <span className="font-mono text-primary text-xl">{formatCurrency(selectedEmployeeDetail.montoNeto)}</span>
                                </div>
                            </div>
                            <Separator />
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-center">
                                    <div className="border p-2 rounded">Abril: {formatCurrency(selectedEmployeeDetail.pagoAbril)}</div>
                                    <div className="border p-2 rounded">Agosto: {formatCurrency(selectedEmployeeDetail.pagoAgosto)}</div>
                                    <div className="border p-2 rounded">Diciembre: {formatCurrency(selectedEmployeeDetail.pagoDiciembre)}</div>
                                </div>
                            </div>
                        </div>
                    )}
